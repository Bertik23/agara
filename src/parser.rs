...
fn parse_init(tokens: &mut Vec<Token>) -> ASTTree{
    ...
    match tokens.pop().unwrap() {
        Token::Ident(id, _) => {lhs = ASTTree::Variable(id);}
...
fn parse_function_def(tokens: &mut Vec<Token>) -> ASTTree{
    ...
    if let Token::LParen(_) = tokens.pop().unwrap() {} else {panic!("Expected `(`")}
    while let Token::Ident(name, _) = tokens.pop().unwrap(){
        params.push(ASTTree::Variable(name))
    }

    if let Token::StartBlock(_) = tokens.pop().unwrap() {} else {panic!("Expected code block")}
    dbg!(&tokens);
...
fn parse_call(tokens: &mut Vec<Token>) -> Vec<ASTTree>{
    let mut params = vec![];
    tokens.pop();
    while if let Token::RParen(_) = tokens.last().unwrap() {false} else {true} {
...
fn parse_primary(tokens: &mut Vec<Token>) -> ASTTree{
    ...
    match tokens.last().unwrap(){
        ...
        Token::Ident(_, _) => {lhs = parse_ident(tokens);}
        Token::String(_, _) => {lhs = parse_string(tokens);}
...
pub fn parse(tokens: &mut Vec<Token>) -> Vec<ASTTree>{
    ...
    loop {
        match tokens.last().unwrap() {
            ...
            Token::EndBlock(_) => {dbg!("Hello", tokens.pop(), &ast); return ast}
